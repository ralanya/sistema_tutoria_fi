<?php
  ...
  $Fecha1 = "";
  $Fecha2 = "";                
  if(isset ($_POST['txtFIniEst4']) && ($_POST['txtFFinEst4']))
  {
    $FIniEst = $_POST["txtFIniEst4"];
    $Fecha1 = $FIniEst;
    list($anio1,$mes1,$dia1) = explode("-",$FIniEst);
    $FFinEst = $_POST["txtFFinEst4"];
    $Fecha2 = $FFinEst;
    ...
    if($GSEst=="Todos" and $Cat!="Todos"){ 
      $sql2 = "SELECT * FROM estudiante e, incidencia i, tipo_incidencia ti
            ...
            AND i.InfoIncid IN $IdTipo AND i.FechaIncid BETWEEN '$FIniEst' AND '$FFinEst' AND ti.NombTipoIncid ='$Cat'";
    }
    else if($Cat=="Todos" and $GSEst!="Todos"){ 
      $sql2 = "SELECT * FROM estudiante e, incidencia i, tipo_incidencia ti
            WHERE e.IdEstudiante = i.IdEstudiante AND i.IdTipoIncid = ti.IdTipoIncid AND EstIncid = '1'
            AND i.InfoIncid IN $IdTipo AND i.FechaIncid BETWEEN '$FIniEst' AND '$FFinEst' AND GSEst = '$GSEst'";
    }
    else if($Cat=="Todos" and $GSEst=="Todos"){ 
      $sql2 = "SELECT * FROM estudiante e, incidencia i, tipo_incidencia ti
            WHERE e.IdEstudiante = i.IdEstudiante AND i.IdTipoIncid = ti.IdTipoIncid AND EstIncid = '1'
            AND i.InfoIncid IN $IdTipo AND i.FechaIncid BETWEEN '$FIniEst' AND '$FFinEst'";
    }
    else if($Cat!="Todos" and $GSEst!="Todos"){
      $sql2 = "SELECT * FROM estudiante e, incidencia i, tipo_incidencia ti
            WHERE e.IdEstudiante = i.IdEstudiante AND i.IdTipoIncid = ti.IdTipoIncid AND EstIncid = '1'
            AND i.InfoIncid IN $IdTipo AND e.GSEst = '$GSEst' AND i.FechaIncid BETWEEN '$FIniEst' AND '$FFinEst' AND ti.NombTipoIncid ='$Cat'";
    }
  ...
  else
  {
    ...
    $sql2 = "SELECT e.IdEstudiante, i.FechaIncid, i.DescIncid, e.NombEst, e.ApelEst, e.GSEst, e.DniEst FROM estudiante e, incidencia i
            ...
            AND e.GSEst = '$GSEst' AND i.FechaIncid BETWEEN '$FIniEst' AND '$FFinEst'";
    $fila2 = mysqli_query($cn,$sql2);
      ...
      <div class="row">
        <div class="col-xs-12">
          ...
          <div class="box">
            ...
            <div class="box-body">            
            Categoría: <input type="text" class="form-control" value="<?php echo $Cat; ?>" readonly="readonly" style="font-size:16px"/> 
            </div>
            <div class="box-body">            
            ...
            <div class="box-body">
              ...
              <input type="hidden" maxlength="8" name="lstTipo2" id="lstTipo2" class="form-control" value="<?php echo $Cat; ?>" />
              <input type="hidden" maxlength="8" name="txtFIniEst4" id="txtFIniEst4" class="form-control" value="<?php echo $Fecha1; ?>" />
              <input type="hidden" maxlength="8" name="txtFFinEst4" id="txtFFinEst4" class="form-control" value="<?php echo $Fecha2; ?>" />
              <script src="../dist/js/jquery-1.11.1.min.js"></script>
              <script>
              function reporteIncidenciaCategoria(){
              var fini4 = $('#txtFIniEst4').val();
              var ffin4 = $('#txtFFinEst4').val();
              var opcion = $('#lstOpcion2').val();         
              var tipo = $('#lstTipo2').val();                
                if(fini4==""){                
                  ...
                  window.open('../pages/reporteincidenciacatword.php?fini='+fini4+'&ffin='+ffin4+'&opcion='+opcion+'&tipo='+tipo);
                }              
              }            
              </script>
              <div class="box-footer">                
                <a class="btn btn-primary" href="reporte"><img src="images/regresa.png" /> Regresar</a>
                <a class="btn btn-primary" target="_blank" href="javascript:reporteIncidenciaCategoria();"><img src="images/Word2013.png"/> Generar Word</a> 
              </div>
            </div>
            <!-- /.box-body -->
          </div>
