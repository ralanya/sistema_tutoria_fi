<?php
  ...
  $sql_deri = mysqli_query($cn,"SELECT * FROM derivacion WHERE IdDerivacion = '$IdDerivacion'");
          ...
          <div class="box">
            ...
            FECHA: <input type="text" id="archivo" name="txtAlumno" class="form-control" value="<?php echo $fecha1; ?>" readonly="readonly" style="font-size:16px"> <br>
                ...
                <div class="form-group">
                    ...
                    <textarea rows="2" value="" name="txtMotiDeri" class="form-control" placeholder="Por favor escriba el motivo de derivación..." readonly=""><?php echo $r_deri["DescDeri"] ?></textarea>             
                  </div>
                  <div class="form-group">
                    <label>ACCIONES YA REALIZADAS</label>
                    <textarea rows="2" value="" name="txtAcciRea" class="form-control" placeholder="Por favor escriba las acciones ya realizadas..." readonly=""><?php echo $r_deri["AcreDeri"] ?></textarea>             
                  </div>

                  <div class="form-group">
                    <label>PERSONAL QUE RECIBE EL CASO</label>
                    <select id="lstPsicologo" name="lstPsicologo" class="form-control" style="width: 44%" disabled="">                      
                    <?php
                    $sql3 = mysqli_query($cn,"SELECT * FROM derivacion d, personal p, tipo_personal tp WHERE d.IdPersona = p.IdPersonal AND p.IdTipoPers = tp.IdTipoPers AND d.IdDerivacion = '$IdDerivacion'");
                    while($r3 = mysqli_fetch_array($sql3))
                    {
                      echo '<option value="'.$r3["IdPersonal"].'">'.utf8_encode($r3["ApelPers"]).', '.utf8_encode($r3["NombPers"]).' ('.utf8_encode($r3["NombTipoPers"]).')</option>';
                    }
                    ?>                      
                    </select>               
                  </div>
            ...
            <div class="box-body">             
              ...
              <div class="box-footer">
                ...
                <a class="btn btn-primary" target="_blank" href="reportes/ficha-derivacion?id=<?php echo $r_deri["IdDerivacion"] ?>"><img src="images/print.png" /> Imprimir</a>
              </div>
              </form>
            </div>
            <!-- /.box-body -->
